...
//   - Per-thread signal mask / ptrace style state
//
// runtime.LockOSThread() pins the current G to its current M, preventing
// migration. It preserves thread identity; it does not opt out of Go's
// scheduler or turn goroutines into raw 1:1 threads. It is also not a mutex:
// use normal synchronization primitives for shared memory.
//
// Run:
...
func demo1_ThreadIdentity() {
	...
	fmt.Println("  LockOSThread preserves thread identity; it is not a CPU")
	fmt.Println("  performance switch.")
	fmt.Println("  It is also not a substitute for a mutex or channel.")
	fmt.Println()
}
...
//   }
//
// The public guarantee to rely on is the one in the runtime docs:
//   - All init functions are run on the startup thread.
//   - Calling LockOSThread from an init function causes main to be invoked on
//     that thread.
//
// That is why GUI frameworks lock in init(). Without that init-time lock, do
// not assume later calls are still running on the startup thread.
//
// This remains true in containers and cloud VMs: thread identity still works,
// but CPU placement is still controlled by the guest OS, host OS, and
// hypervisor.
//
...
func demo3_MainGoroutine() {
	...
	fmt.Println()
	fmt.Println("  ┌─────────────────────────────────────────────────────────┐")
	fmt.Println("  │  WHAT THE RUNTIME GUARANTEES                           │")
	fmt.Println("  │                                                        │")
	fmt.Println("  │  All init() functions run on the startup thread.       │")
	fmt.Println("  │  If init() calls LockOSThread, main() is invoked on    │")
	fmt.Println("  │  that same startup thread.                             │")
	fmt.Println("  │                                                        │")
	fmt.Println("  │  Use that documented guarantee for main-thread APIs.   │")
	fmt.Println("  └─────────────────────────────────────────────────────────┘")
	fmt.Println()
}
...
//
// Detection strategy:
//   runtime/metrics metric /sched/threads/total:threads tracks the current
//   count of live threads owned by the Go runtime.
//   When a locked goroutine exits, its M is destroyed → count can drop.
//   Under load, the runtime creates replacements → count can rise.
...
func demo5_MLeakDetection() {
	...
	fmt.Println("  destroyed (not returned to pool). The runtime metric")
	fmt.Println("  /sched/threads/total:threads tracks live Go-owned threads.")
	fmt.Println()

	// --- Baseline ---
	baseline := liveThreadCount()
	...
	fmt.Println("    samples := []metrics.Sample{{Name: \"/sched/threads/total:threads\"}}")
	fmt.Println("    ticker := time.NewTicker(30 * time.Second)")
	fmt.Println("    var lastCount int")
	fmt.Println("    for range ticker.C {")
	fmt.Println("        metrics.Read(samples)")
	fmt.Println("        count := int(samples[0].Value.Uint64())")
	...
	fmt.Println("  ├──────────────────────────────────────────────────────────┤")
	fmt.Println("  │  1. Monitor /sched/threads/total:threads                │")
